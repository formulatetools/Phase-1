...
async function generateSectionBased(options: {
  ...
  const { target, targetSelector, worksheetTitle, showBranding, logoDataUrl } = options
  ...
  for (let i = 0; i < sectionImages.length; i++) {
    ...
    if (currentPageY + neededHeight <= CONTENT_HEIGHT) {
    ...
    } else if (img.heightMm <= CONTENT_HEIGHT) {
  ...
  const totalPages = pages.reduce((count, pageSlots) => {
    ...
    for (const slot of pageSlots) {
      ...
      if (slot.y + img.heightMm > CONTENT_HEIGHT) {
        // This section overflows — calculate how many extra pages it needs
        const overflow = (slot.y + img.heightMm) - CONTENT_HEIGHT
        return count + Math.ceil(overflow / CONTENT_HEIGHT)
      }
    }
    return count
  }, pages.length)
  ...
  for (let pageIdx = 0; pageIdx < pages.length; pageIdx++) {
    ...
    globalPageNum++
    addHeader(pdf, globalPageNum, totalPages, worksheetTitle, logoDataUrl)
    addFooter(pdf, showBranding)
    ...
    for (const slot of slots) {
      ...
      if (slot.y + img.heightMm <= CONTENT_HEIGHT) {
        ...
        while (remainingMm > 0) {
          ...
          const yPos = firstSlice ? CONTENT_Y + slot.y : CONTENT_Y
...
async function generateLegacy(options: {
  ...
  for (let page = 0; page < totalPages; page++) {
    ...
    addHeader(pdf, page + 1, totalPages, worksheetTitle, logoDataUrl)
    addFooter(pdf, showBranding)

    const sourceY = (page * CONTENT_HEIGHT / ratio) * SCALE
    const sourceHeight = Math.min(
      (CONTENT_HEIGHT / ratio) * SCALE,
      imgHeight - sourceY,
    )
    ...
    pdf.addImage(pageImgData, 'PNG', ML, CONTENT_Y, CONTENT_WIDTH, pageHeight)
  }
