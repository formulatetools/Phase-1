...
// ─── Icons (extracted so they can be re-used in both desktop & mobile) ──────
...
export function SidebarNav({
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem('formulate_nav_sections')
  ...
  const toggleSection = (id: string) => {
    setCollapsedSections((prev) => {
      ...
      try {
        localStorage.setItem('formulate_nav_sections', JSON.stringify([...next]))
      } catch { /* noop */ }
      return next
    })
  }
  ...
  const sections: NavSection[] = [
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'Feature Requests', href: '/feature-requests', icon: icons.featureRequests },
      ],
    },
  ]
  ...
  const sidebarContent = (
    <>
      ...
      <nav aria-label="Main" className="flex-1 overflow-y-auto px-3 py-3" data-tour="sidebar-nav">
          ...
          return (
            <div key={section.id} className="mb-1">
              ...
              <button
                ...
                className="flex w-full items-center justify-between rounded-md px-3 py-2 text-[10px] font-semibold uppercase tracking-wider text-primary-400 transition-colors hover:text-primary-600 dark:text-primary-500 dark:hover:text-primary-400"
              ...
              >
                <div className="overflow-hidden">
                  <div className="space-y-0.5 pb-1">
                    {section.items.map((item) => renderNavLink(item, isActive(item.href)))}
                  </div>
                </div>
              </div>
            </div>
        ...
        {/* Standalone items below grouped sections */}
        <div className="mt-1 border-t border-primary-100 pt-2 space-y-0.5">
          {standaloneItems.map((item) => renderNavLink(item, isActive(item.href)))}
        </div>
      </nav>
        ...
        <div className="flex">
          {mobileTabItems.map((item) => {
            ...
            return (
              ...
              >
                <span className={`relative ${active ? 'text-brand' : ''}`}>
                  {item.icon}
                  {/* Badge on Clients tab */}
                  {item.href === '/clients' && pendingReviewCount > 0 && (
