...
export default async function AdminRevenuePage() {
  ...
  return (
    <div className="px-4 py-8 sm:px-8 lg:px-12">
      ...
      {/* ── Stat cards ──────────────────────────────────────────────── */}
      <div className="mb-8 grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        {/* MRR */}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="border-b border-primary-50 bg-primary-50/50">
              <tr>
                <th className="px-3 sm:px-6 py-3 font-medium text-primary-500">Event</th>
                <th className="hidden sm:table-cell px-3 sm:px-6 py-3 font-medium text-primary-500">Tier</th>
                <th className="px-3 sm:px-6 py-3 font-medium text-primary-500">Status</th>
                <th className="px-3 sm:px-6 py-3 font-medium text-primary-500">Date</th>
              </tr>
            </thead>
                ...
                (e: { id: string; action: string; metadata: Record<string, string> | null; created_at: string }) => {
                  ...
                  return (
                    <tr key={e.id} className="hover:bg-primary-50/50 transition-colors">
                      <td className="px-3 sm:px-6 py-3 font-medium capitalize text-primary-900">
                        {e.action === 'create' ? 'New subscription' : e.action === 'update' ? 'Updated' : e.action === 'delete' ? 'Cancelled' : e.action}
                      </td>
                      <td className="hidden sm:table-cell px-3 sm:px-6 py-3 text-primary-600">
